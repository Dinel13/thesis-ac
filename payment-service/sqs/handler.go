...
func (h sqsHandler) SendMsg(msgId *string, data map[string]interface{}) (*string, error) {
	svc := sqs.New(h.sess)
...
func (h sqsHandler) WaitMsgSqs() {
	for {
		...
		_, err = h.SendMsg(idReq, map[string]interface{}{"ispay": payment.IsPay})
		if err != nil {
			fmt.Println(err)
			continue
		}
